...
@implementation NW_MBProgressHUD
...
- (void)setMode:(NW_MBProgressHUDMode)newMode {
...
- (void)setProgress:(float)newProgress {
    ...
    if (mode == NW_MBProgressHUDModeDeterminate) {
		if ([NSThread isMainThread]) {
			[self updateProgress];
			[self setNeedsDisplay];
		} else {
...
- (void)updateProgress {
    [(NW_MBRoundProgressView *)indicator setProgress:progress];
}
...
- (void)updateIndicators {
    ...
    if (mode == NW_MBProgressHUDModeDeterminate) {
        self.indicator = [[[NW_MBRoundProgressView alloc] init] autorelease];
    }
    else if (mode == NW_MBProgressHUDModeCustomView && self.customView != nil){
...
+ (NW_MBProgressHUD *)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
	NW_MBProgressHUD *hud = [[NW_MBProgressHUD alloc] initWithView:view];
	[view addSubview:hud];
	[hud show:animated];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
	...
	for (UIView *v in [view subviews]) {
		if ([v isKindOfClass:[NW_MBProgressHUD class]]) {
			viewToRemove = v;
		}
	}
	if (viewToRemove != nil) {
		NW_MBProgressHUD *HUD = (NW_MBProgressHUD *)viewToRemove;
...
- (id)initWithView:(UIView *)view {
	...
	if (!view) {
		[NSException raise:@"NW_MBProgressHUDViewIsNillException" 
					format:@"The view used in the NW_MBProgressHUD initializer is nil."];
	}
...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self) {
        ...
        self.animationType = NW_MBProgressHUDAnimationFade;
        self.mode = NW_MBProgressHUDModeIndeterminate;
        self.labelText = nil;
...
- (void)showUsingAnimation:(BOOL)animated {
    self.alpha = 0.0f;
    if (animated && animationType == NW_MBProgressHUDAnimationZoom) {
    ...
    if (animated) {
        ...
        self.alpha = 1.0f;
        if (animationType == NW_MBProgressHUDAnimationZoom) {
...
@implementation NW_MBRoundProgressView
