...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        {
            contentImageView = [[UIImageView alloc] initWithFrame:CGRectMake(kDistanceFromLeftEdge, (self.frame.size.height - kBoxEdgeSize)/2, kBoxEdgeSize, kBoxEdgeSize)];
...
- (void)layoutSubviews
{
    ...
    // Make sure the frame is where we want it (because we resize the standard TVC in the TableViewController the image will be off a bit otherwise)
    ...
    self.textLabel.frame = CGRectMake(tlFrame.origin.x + kBoxEdgeSize + kDistanceFromLeftEdge, tlFrame.origin.y, contViewFrame.size.width - (2*kDistanceFromLeftEdge + kBoxEdgeSize + 7), tlFrame.size.height);

    // Shrink the detailTextLabel view by size of boxEdgeSize, and shift that much right plus offset from leftEdge
    self.detailTextLabel.frame = CGRectMake(dtlFrame.origin.x + kBoxEdgeSize + kDistanceFromLeftEdge, dtlFrame.origin.y, contViewFrame.size.width - (2*kDistanceFromLeftEdge + kBoxEdgeSize + 7), dtlFrame.size.height);
}
@end
